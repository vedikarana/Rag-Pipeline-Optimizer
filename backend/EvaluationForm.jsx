...
function EvaluationForm({ onEvaluate, isEvaluating }) {
  ...
  const handleSubmit = () => {
    ...
    const activeCriteria = Object.entries(customCriteria)
      .filter(([_, isActive]) => isActive)
      .map(([criterion]) => criterion);
    
    onEvaluate(validQuestions, activeCriteria);
  };
    ...
    <div className="space-y-6">
      ...
      {showAdvanced && (
        <div className="bg-slate-800 rounded-lg p-6 space-y-4">
          ...
          {Object.entries(customCriteria).map(([criterion, isActive]) => (
            <label key={criterion} className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-white capitalize">{criterion}</span>
            </label>
          ))}
        </div>
      )}
